...
// SOCKS proxy address (you'll need one from your network or VPN provider)
const proxyUrl = 'socks5h://192.168.13.176:1080'; // Note: NOT http://...
...
const transporter = nodemailer.createTransport({
  ...
  agent, // Use the proxy agent for outgoing connections
});

// Regex for allowed email domains
const allowedEmailRegex = /^[a-zA-Z0-9._%+-]+@(nic\.in|cdac\.in|gov\.in|jk\.gov\.in)$/;
...
exports.sendOtp = async (req, res) => {
  ...
  if (!allowedEmailRegex.test(email)) {
    return res.status(400).json({
      message: 'Invalid email domain. Please use a .nic.in, .cdac.in, or .gov.in email address.',
      type: 'error',
    });
  }
